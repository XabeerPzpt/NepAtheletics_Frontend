...
const AddCategory = ({fetchData,onCancel}) => {
    ...
    const handleSubmit = async(e) =>{
        ...
        try {
            const response = await axios.post(`https://localhost:7106/api/Categories/add_category?name=${encodeURIComponent(name)}`,
                null,
                {
                    headers: {
    ...
    return (
        ...
        <EditAndUpdateForm heading="Add Category"  onSubmit={handleSubmit} buttonName="Add" onCancel={onCancel} additionalChildren={<ToastContainer />}>
